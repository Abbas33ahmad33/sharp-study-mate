      ...
      <main className="container mx-auto px-4 py-6 pb-24 space-y-8">
        {isLoading ? (
          ...
          <>
            ...
            {/* My Institutes Section - Enhanced */}
            {myInstitutes.length > 0 && (
              <Card className="border-0 shadow-xl shadow-card/20 bg-card animate-slide-up" style={{ animationDelay: '0.1s' }}>
                ...
                <CardContent>
                  <div className="space-y-3">
                    {myInstitutes.map((enrollment) => (
                      <div key={enrollment.id} className="flex items-center justify-between p-4 rounded-2xl bg-muted/40 border border-border/50 hover:border-primary/30 transition-colors">
                        <div className="flex items-center gap-4">
              ...
              >
                <Card className={`overflow-hidden transition-all duration-300 border-0 shadow-xl ${expandedSubject === 'upcoming-exams' ? 'shadow-secondary/20' : 'shadow-card/20'
                  ...
                  <CollapsibleTrigger asChild>
                    <div className="cursor-pointer transition-all duration-300 hover:bg-muted/20">
                ...
                {subjects.map((subject, subjectIndex) => {
                  ...
                  return (
                    ...
                    >
                      <Card
                        className={cn(
                          "group relative transition-all duration-500 border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden animate-slide-up bg-white dark:bg-slate-900 active:scale-[0.99] touch-none",
                          colorScheme.bg,
                          isExpanded
                            ? cn("shadow-xl sm:shadow-2xl rounded-[2rem] scale-[1.01] ring-1", colorScheme.border.replace('border-', 'ring-'))
                            : "hover:shadow-md rounded-2xl hover:border-primary/30"
                        )}
                            ...
                            <div className="p-6 sm:p-8">
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-5 sm:gap-7">
                                  <div className={cn(
                                    "w-16 h-16 sm:w-20 sm:h-20 rounded-[1.5rem] flex items-center justify-center transition-all duration-500 bg-white dark:bg-slate-800 shadow-sm relative overflow-hidden group-hover:scale-105 group-hover:shadow-md group-active:scale-95",
                                    isExpanded && "rounded-[2rem] scale-110 shadow-lg"
                                  )}>
                                    <div className="absolute inset-0 bg-slate-50/50 dark:bg-slate-700/50 opacity-50" />
                                    <BookOpen className={cn("w-8 h-8 sm:w-10 sm:h-10 transition-transform duration-500 relative z-10", colorScheme.icon, isExpanded && "scale-110")} />
                                  </div>
                                  <div>
                                    <h3 className={cn(
                                      "text-2xl sm:text-3xl font-bold text-slate-900 dark:text-slate-100 tracking-tight transition-all duration-300",
                                      isExpanded && "text-primary"
                                    )}>
                                      {subject.name}
                                    </h3>
                                    <div className="flex items-center gap-4 mt-1.5 sm:mt-2">
                                      <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400 text-[11px] sm:text-xs font-bold uppercase tracking-wider">
